...
4. **ローカルUIサーバーのポート**
   - このマシンでは `3002`（`.env` の `PORT=`）に固定している。**新しいポート番号を選ぶ前に
     必ず他のローカルプロジェクトと衝突しないか確認すること** — このマシンでは `3000` が
     `encuore-site`、`3001` が `adb-mcp`（Photoshop等のUXPプロキシ、複数プラグインの権限に
     ポート番号がハードコードされていて変更コストが高い固定枠）で既に使用中。
     `lsof -i :<port>` で今動いているプロセスは確認できるが、それだけでは不十分
     （adb-mcpのように普段は落ちていて必要な時だけ起動するプロセスもある）。他のローカル
     プロジェクトのポート割り当てもあわせて確認してから空いている番号を選ぶこと。
...
`http://localhost:3002` を開く（このマシンでのポート、3節参照）。1画面で完結する：
